...
//-----------------------------------------------------------------------------
// ★追加: カードスタイル定義 (レイアウト設定)
//-----------------------------------------------------------------------------
TMPlugin.Card.Layouts = {
    // ■ 通常レイアウト (デフォルト)
    'default': {
        framePrefix: 'c_frame_', // 枠画像の接頭辞 (c_frame_0.png 等)
    ...
    'promo': {
        framePrefix: 'p_frame_', // 別の枠画像 (p_frame_0.png 等を用意すること)
        drawFrame: true,
	...
	Game_CardBattle.prototype.update = function() {
		...
		if (this.isPhaseUpdateEnable()) {
			if (this._phase === 0) {
				...
				// ■変更: ここでポップせず、終了状態(phase 5)を維持する
	...
	Game_CardBattle.prototype.updateEndPhase = function() {
		...
		if ((this._cardDeath)){ //直前で死んでるときはラウンド仕切り直し
			...
			AudioManager.playSe({"name":"Move1","volume":90,"pitch":120,"pan":0}) //ラウンド進行SE
    	}else{ //死んでない場合    	
	...
	Game_Card.prototype.initialize = function(cardId) {
    	...
    	if (typeof cardId === 'object' && cardId !== null) {
        	// オブジェクト（アイテムデータ）が渡された場合
        	this._item = cardId;
        	this._cardId = cardId.id;
        	this._hide = false;
    	} else {
        	// 従来通り ID（数字）が渡された場合
        	this._cardId = Math.abs(cardId || 0);
	...
	window.Game_Card = Game_Card; // 追加：Game_Cardも外部公開する
	...
	Sprite_Card.prototype.initialize = function(card) {
    	...
    	this._foilSprite.blendMode = Graphics.BLEND_SCREEN; // 合成方法の指定。 BREND_ADD, BREND_NORMAL, BLEND_SCREEN
	...
	//-----------------------------------------------------------------------------
    // ★カードスタイル：Sprite_Card.prototype.loadCardBitmap の上書き
    //-----------------------------------------------------------------------------
    ...
    Sprite_Card.prototype.createCardBitmap = function() {
        ...
        // やられ状態の描画処理（変更なし）
        if (this._knockout) {
            // (省略: 元のコードのknockout処理をそのまま使ってください。長くなるため割愛しますが、必要なら元のコードを維持)
            // 元のコードをコピペしてください ↓
            this._foilSprite.visible = false; // 裏面などはFoil非表示
        ...
        if (!this._hide && this._card.id() > 0 && TMPlugin.Card.UseAutoText) {
            var layout = this._card.layout(); // ★レイアウト取得

            // 背景とカード絵
            this.bitmap.blt(this._backBitmap, 0, 0, 192, 288, 0, 0);
            this.bitmap.blt(this._cardBitmap, 0, 0, 192, 288, 0, 0);
            
            // ★枠の描画 (設定で有効かつ画像がある場合のみ)
            ...
            if (this._card.type() < 4) { // アイテムカード以外
                // 攻撃力
	...
	Sprite_Card.prototype.update = function() {
    	...
    	if (this._foilSprite && this._foilSprite.visible && this._foilSprite.bitmap) {
        	// 時間経過で透明度をふわふわさせる（最大値を下げると色が変わる。）
        	var speed = 0.02; // 数値を大きくすると速くなります
        	var time = Graphics.frameCount * speed;
        	// --- Opacity の計算式 ---
        	// Math.sin(time) は -1.0 ～ 1.0 の値を返します。
        	// これに +1 して /2 することで、0.0 ～ 1.0 の範囲に変換します。
        	var wave = (Math.sin(time) + 1) / 2;
        
        	// Y座標をわずかに動かすアニメーション (例: -10 ～ 10 の範囲)
		    //this._foilSprite.y = Math.sin(time/2.0) * 10;
		    
		    // スケールを 1.0 ～ 1.1 の間で変化させる
    		var scaleWave = 0.9 + ((Math.sin(time) + 1) / 2) * 0.1;
    		this._foilSprite.scale.x = scaleWave;
    		this._foilSprite.scale.y = scaleWave;
        
        	// 最小値 10、最大値 255 の範囲で適用
        	var minOpacity = 10;
        	var maxOpacity = 230;
