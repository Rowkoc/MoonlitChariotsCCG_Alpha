...
(function() {
        ...
        if (command === 'Gacha') {
            switch (args[0]) {
                ...
                case 'clear':
                    ...
                    break;
                case 'alladd': //独自追加。全部のアイテムをガチャ図鑑に。
 	                for (var i = 2; i < $dataItems.length; i++) {
					  if ($dataItems[i] && $dataItems[i].name !== '') {
					    console.log('addIndex='+ i);
					    $gameSystem.addToGacha(args[1], i);
					  }
				    }
                    break;
            }
    ...
    Window_GachaResult.prototype.drawResultImage = function(param, index) {
        ...
        var sx = param.original ? 0 : param.x ? param.x - origin : origin; //三項演算子。 0 or x-サイズ or サイズ。
