//=============================================================================
// ROW_ShopItemPicture_DebugPlus.js
// ショップで選択中のアイテムに応じてピクチャを表示（存在チェック付き）
//=============================================================================

/*:
 * @plugindesc ショップで選択中のアイテムに応じてピクチャを表示（存在チェック付きデバッグ版）画像は img/pictures に配置してください。
 * @author Rowkoc
 *
 * @param PictureId
...
(function() {
    const parameters = PluginManager.parameters('ROW_ShopItemPicture_DebugPlus');
    ...
    Window_ShopBuy.prototype.updateHelp = function() {
        ...
        if (!item) {
            console.log("[ShopPicture] No item selected.");
            return;
        }
        ...
        if (useNote) {
            ...
            if (match) {
                ...
                console.log(`[ShopPicture] Found <pic:...> tag in note: ${picName}`);
            }
        }

        if (!picName && item.name) {
            picName = item.name.trim();
            console.log(`[ShopPicture] Using item name as picture: ${picName}`);
        }

        if (picName) {
            const bitmap = ImageManager.loadPicture(picName);
            bitmap.addLoadListener(() => {
                console.log(`[ShopPicture] ✅ Picture '${picName}' loaded. Displaying at (${posX}, ${posY})`);
                $gameScreen.showPicture(picId, picName, 1, posX, posY, 100, 100, 255, 0);
            });
            bitmap.addLoadErrorListener(() => {
                console.log(`[ShopPicture] ❌ Picture '${picName}' NOT FOUND in img/pictures`);
            });
        } else {
            console.log("[ShopPicture] No picture name determined.");
        }
    };

    const _Scene_Shop_terminate = Scene_Shop.prototype.terminate;
    Scene_Shop.prototype.terminate = function() {
        console.log("[ShopPicture] Erasing picture.");
