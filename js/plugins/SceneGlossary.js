...
(function() {
    ...
    Window_Glossary.prototype.drawItem = function(pageIndex, noSound) {
        ...
        var bitmap    = this.getGlossaryBitmap(pageIndex); //画像を取得してビットマップに入れる。
    ...
    Window_Glossary.prototype.getGlossaryBitmap = function(index) { //表示する画像を取得。
        var pictureName = this.getPictureName(index);
        var enemy       = this.getEnemyData(index);
        
        if (pictureName) { //画像名が入っているなら。
	        return ImageManager.loadPicture(pictureName, 0); //イメージマネージャーで画像を取得。
        } else {
            if (enemy) {
    ...
    Window_Glossary.prototype.drawItemSub = function(bitmap, listIndex, pageIndex) {
        ...
        switch (this.getPicturePosition()) { //画像位置の指定により切り替え。
            case 'top':
                if (!textPos) {
