...
(function() {
	...
	Game_CardBattle.prototype.updateJudgmentPhase = function() {
		...
		} //this._cardDeath が抜けて変わってるので確認(素早さ処理入れているかも）
		if (target.hp === 0){
			target.knockout();
			this._cardDeath =true	
		}
	...
	Game_Deck.prototype.initCardStatus = function() {
		...
		var card = this.card();
		this._hp = card.hp();
		this._atk = card.atk().clamp(1, $gameCardBattle.maxAtk); //カードの攻撃力を最大攻撃力にクランプ
		//row カードスピードの追加
		this._spd = card.spd();
		console.log("card.name="+card.name()+", [hp,atk,spd]=["+this._hp+","+this._atk+","+this._spd+"]");
		//Row追記；種族配列を追加する。
	...
	Sprite_Card.prototype.createCardBitmap = function() {
		...
		if (!this._hide && this._card.id() > 0 && TMPlugin.Card.UseAutoText) {
			...
			this.bitmap.outlineColor = '#dddddd';
	...
	Spriteset_CardBattle.prototype.createNumberSprites = function() {
		...
		//この辺から追加表示内容(Row)
		// プレイヤーHPラベル
		let label = new Sprite(new Bitmap(64, 24));
		label.bitmap.fontSize = 18;
		label.bitmap.drawText("HP", 0, 0, 64, 24, "center");
		label.x = this._playerHpSprite.x;
		label.y = this._playerHpSprite.y - 40;
		this._baseSprite.addChild(label);

		// プレイヤーATKラベル
		label = new Sprite(new Bitmap(64, 24));
		label.bitmap.fontSize = 18;
		label.bitmap.drawText("ATK", 0, 0, 64, 24, "center");
		label.x = this._playerAtkSprite.x;
		label.y = this._playerAtkSprite.y - 40;
		this._baseSprite.addChild(label);
		...
		label.bitmap.fontSize = 18;
		label.bitmap.drawText("HP", 0, 0, 64, 24, "center");
		label.x = this._enemyHpSprite.x;
		label.y = this._enemyHpSprite.y - 40;
		this._baseSprite.addChild(label);
		...
		label.bitmap.fontSize = 18;
		label.bitmap.drawText("ATK", 0, 0, 64, 24, "center");
		label.x = this._enemyAtkSprite.x;
		label.y = this._enemyAtkSprite.y - 40;
		this._baseSprite.addChild(label);
...
this._playerHpSprite.setChangeStartHandler(() => {
    this.startDamageDisplay("player", $gameCardBattle._damage);
});
this._enemyHpSprite.setChangeStartHandler(() => {
    this.startDamageDisplay("enemy", $gameCardBattle._damage);
});
};
...
Spriteset_CardBattle.prototype.updateNumberSprites = function() {
    this._playerHpSprite.update();
    this._enemyHpSprite.update();
    this._playerAtkSprite.update();
    this._enemyAtkSprite.update();

    const label = (this._damageTarget === "player") ? this._damageLabelPlayer : this._damageLabelEnemy;

    const isAnimating =
        this._playerHpSprite.isChanging() ||
        this._enemyHpSprite.isChanging();
...
Sprite_Number.prototype.update = function() {
    if (this._value !== this._targetValue) {
        ...
        this._value += Math.sign(diff);
        this.refresh();

        if (!this._changingStarted) {
            this._changingStarted = true;
            if (this._onChangeStart && typeof this._onChangeStart === "function") {
                this._onChangeStart();
            }
        }
    } else {
        this._changingStarted = false;
    }
};
