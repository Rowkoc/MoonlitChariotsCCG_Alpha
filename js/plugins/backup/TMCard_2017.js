...
(function() {
  ...
  Game_CardBattle.prototype.updateEndPhase = function() {
    ...
    if (this._turn === this._firstAttack) this._turnCount += 1;//先攻ならターンカウント+1
  ...
  Game_CardBattle.prototype.useSkill = function(user, index, target, active) {
    console.log("useSkill:"+ index +", active:"+active);
    var skill = user.skill(index);//スキル内容を取得
    if (!skill) return; //スキルが存在するか
    console.log("Skill:"+ skill.name+", "+this.meetsConditions(skill, user, index, target, active));
  ...
  console.log("●カード情報をセット")
    ...
    this._skills[this.size()+1] = card.unitSkillB(); //固有スキル2
