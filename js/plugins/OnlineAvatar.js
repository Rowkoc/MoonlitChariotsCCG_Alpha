 ...
 * @default AIzaSyDY5wpLUqd1MtKv7LKN7_lt8RegRn5pCaY
 *
 * @param authDomain
 * @desc FirebaseのauthDomain。各自コピペしてきてね
 * @default monnlitchariotstest.firebaseapp.com
 *
 * @param databaseURL
 * @desc FirebaseのdatabaseURL。各自コピペしてきてね
 * @default https://monnlitchariotstest-default-rtdb.asia-southeast1.firebasedatabase.app
 *
 * @param avatarEvent
 ...
 * @default 20
 *
 * @param roomIdVariable
 * @desc ルーム番号指定時に参照する変数です。
 * @default 5
 *
 *
 * @help
...
(function() {
	...
	// OnlineManager.url = 'https://www.gstatic.com/firebasejs/live/3.0/firebase.js'; 古いバージョン
	OnlineManager.url = 'https://www.gstatic.com/firebasejs/7.23.0/firebase.js'; //2026/01 のSDK
	...
	OnlineManager.awake = function() {
		...
		ps['roomIdVariable'] = +ps['roomIdVariable'];
	...
	OnlineManager.start = function(userCredential) { //user をuserCredential に変更
		// SDKのバージョンによって戻り値の構造が異なるため、両方に対応させます
    	this.user = userCredential.user || userCredential;
	...
	OnlineManager.connectNewMap = function() {
		...
		// 【改造】変数5番が0ならオフライン、1以上ならその数値をルームIDとして接続
        var roomId = $gameVariables.value(this.parameters['roomIdVariable']); 

		if ((!$dataMap.meta || $dataMap.meta.avatar_off) || roomId <= 0) {
			this.mapRef = null;
			this.selfRef = null;
        ...
        // マップIDの後ろにルームIDを付与して、部屋を区別する
		this.mapRef = firebase.database().ref('map' + $gameMap.mapId().padZero(3) + '_' + roomId);
	...
	//【対戦台用追加コード】
	// 指定した座標にいるオンラインプレイヤーのデッキコードを取得する関数
	// 引数：targetX(相手のX座標), targetY(相手のY座標), tableId(識別用の台番号)
	// デバッグログ付き：相手のデッキコードを取得する関数
	OnlineManager.getOpponentDeck = function(targetX, targetY, tableId) {
		// ログ：どこを調べているか表示
		console.log("【探索】座標(" + targetX + "," + targetY + ") のテーブルID[" + tableId + "]を探します");

		var events = $gameMap.eventsXy(targetX, targetY);
		if (events.length === 0) {
			console.log("　→ 指定座標にイベントが存在しません");
		}

		for (var i = 0; i < events.length; i++) {
			var e = events[i];
			// それがGame_Avatar（他プレイヤー）かどうか
			if (e instanceof Game_Avatar) {
				if (e.online) {
					console.log("　→ アバター発見: 相手のTableId=" + e.online.tableId + " / Deck=" + e.online.deckCode);
					
					// 判定
					if (e.online.tableId == tableId && e.online.deckCode) {
						console.log("　★マッチング成功！");
						return e.online.deckCode;
					} else {
						console.log("　→ 条件不一致（テーブルID違い、またはデッキコード未設定）");
					}
				} else {
					console.log("　→ アバターはいますが、オンラインデータ(e.online)がまだロードされていません");
				}
			} else {
				console.log("　→ イベントはありますが、プレイヤーアバターではありません（NPCなど）");
			}
		}
		return null;
	};
