...
(function() {
  ...
  Game_CardBattle.prototype.updatePreparationPhase = function() {
    ...
    this.addMessage(0, attacker.userName() + 'のターン'+ this._turnCount);
  ...
  Game_CardBattle.prototype.updateAttackPhase = function() {
    ...
    this.addMessage(4, 0);
    console.log("～～～～～ダメージフェイズ～～～～～");//変動がなくなるまで繰り返す。
    for (;;) {
  ...
  Game_CardBattle.prototype.updateEndPhase = function() {
    ...
    if ((this._turn === this._firstAttack)||(this._cardDeath)){ /ROw追記
      if (this._cardDeath) console.log("死亡更新")
      this._turnCount += 1;   //先攻ならターンカウント+1
      this._cardDeath = false; //カード死亡判定を初期化
      var attacker = this.attackerDeck();
      var target = this.targetDeck();
      //ここで素早さ判定を入れる
      this._turn = this.checkOrders(attacker.card(attacker.lose), target.card(attacker.lose), this._turn);
      this._firstAttack = this._turn;
  ...
  Game_CardBattle.prototype.checkSkill = function(attacker, target, active) {
    ...
    for (var i = attacker.lose; i > 0; i--) { // 1以上の時さかのぼる。
      var tmpNum = i-1
      this.useSkill(attacker, tmpNum, target, active); //i から1少ないインデックスを参照。
      console.log("attacker.state:"+ attacker.isStateAffected(1)+", index_i ="+i);
      //if (attacker.isStateAffected(1)) break; //ステートが戦闘不能だとbreak
    }
    //アイテムのスキル発動（2個分）
    if (attacker.isItemCardReady()) {
      //console.log("アイテム発動");
      this.useSkill(attacker, deckSize + 1 + 2, target, active);
      this.useSkill(attacker, deckSize + 2 + 2, target, active);
    }
  };
  ...
  Game_CardBattle.prototype.useSkill = function(user, index, target, active) {
    ...
    var skill = user.skill(index);//スキル内容を取得
    if (!skill) return; //スキルが存在するか
    ...
    var param = effect[1];
    if (index > user.size() + 2) {//アイテムカードかどうか判断
    ...
    var animTargetInvert = false //アニメーション再生対象を反転するか？
    var message = null;
    switch (effect[0]) {
    case 1:   // 相手に与えるダメージ＋○○
    ...
    var isPlayer = active ? this._turn : !this._turn; //ここがスキルアニメ関連っぽい
    if (animTargetInvert){
    ...
    this.addMessage(0, message); //スキルメッセージを入れている。
    user.useSkill(index);
  };
  ...
  Game_Deck.prototype.initMembers = function() {
    ...
    this._cost = 0;
    this._cards = [];
    this._skills = []; //継承スキルとアイテムスキルのリスト。
    this._skillCount = [];
    this._usedSkills = [];
    this._chargeTime = 0;
    for (var i = 0; i < this._cardIds.length; i++) {
      ...
      this._skills.push(card.partySkill());//継承スキルをセット
      this._skillCount.push(0);
      this._cost += card.cost();
    }
    this._skills.push(null);//キャラとスキルの間をあける。（index=Decksize)
    this._skills.push(null);//unit2個目用 （index=Decksize+1)
    this._skills.push(null);//unit3個目用 （index=Decksize+2)
    this._skillCount.push(0);
    this._skillCount.push(0);
    this._skillCount.push(0);
  ...
  console.log("●カード情報をセット")
	...
	this._faction.push(card.factionB()); //種族をプッシュ 2 
    this._skills[this.size()] = card.unitSkill(); //ここで空き枠(deckSize)に固有スキルを入れている? 
    this._skills[this.size()+1] = card.unitSkillB(); //固有スキル2 たぶん間違ってる。自分の1個下の枠使ってる。
    this._skills[this.size()+2] = card.unitSkillC(); //固有スキル3
  ...
  Game_Card.prototype.faction = function() {
    return this._item ? +this._item.meta.cardFaction : "";
  };
  Game_Card.prototype.factionB = function() {
    return this._item ? +this._item.meta.cardFactionB : "";
  };
  ...
  //固有スキル・継承スキルを3つまで確保（現状unitB以外未使用）
  ...
  Game_Card.prototype.unitSkillC = function() {
    return this._item ? +this._item.meta.unitSkillC : 0;
  };
  
  Game_Card.prototype.partySkillB = function() {
    return this._item ? +this._item.meta.partySkillB : 0;
  };
   Game_Card.prototype.partySkillC = function() {
    return this._item ? +this._item.meta.partySkillC : 0;
  };
  ...
  Sprite_Card.prototype.createCardBitmap = function() {
    ...
    if (!this._hide && this._card.id() > 0 && TMPlugin.Card.UseAutoText) {
      ...
      this.bitmap.blt(this._cardBitmap, 0, 0, this._cardBitmap.width, this._cardBitmap.height, (192-this._cardBitmap.width)/2, 288-this._cardBitmap.height);
      ...
      //種族描画
      ver faction = this.bitmap.drawText(this._card.faction(), 32, 40, 152, 24, 'left');
      ...
      var unitSkillB = $dataSkills[this._card.unitSkillB()];
      if (unitSkillB) {
        this.drawIcon(unitSkillB.iconIndex, 8, 254, 24, 24);
        this.bitmap.drawText(unitSkillB.name, 32, 254, 152, 24, 'left');
      }
      var unitSkillC = $dataSkills[this._card.unitSkillC()];
      if (unitSkillC) {
        this.drawIcon(unitSkillC.iconIndex, 8, 254, 24, 24);
        this.bitmap.drawText(unitSkillC.name, 32, 254, 152, 24, 'left');
      }
      ...
      if (this._card.type() < 4) {
        ...
        this.bitmap.drawText(this._card.spd(), 72, 182, 48, 32, 'center');
      }
    } else {
  ...
  //-----------------------------------------------------------------------------
  // Sprite_TurnCursor アクティブプレイヤー表示の描画
  //
  ...
  Spriteset_CardBattle.prototype.createBackground = function() {//背景
  ...
  Spriteset_CardBattle.prototype.createNumberSprites = function() {
    ...
    //this._playerHpLabel = new Sprite_Text(pos[0], pos[1] - 40, 256, 128, 120, '#000000', 3, '#ffffff');
    //this._baseSprite.addChild(this._playerHpLabel);
  ...
  Spriteset_CardBattle.prototype.createBattleback = function() {
    var margin = 32;
    var x = -this._baseSprite.x - margin;
    var y = -this._baseSprite.y - margin;
    ...
    console.log("x="+this._baseSprite.x+", y="+y+", width="+width+", height="+height)
...
Spriteset_CardBattle.prototype.autotileType = function(z) {
  ...
  Window_DeckEditStatus.prototype.refresh = function() {//デッキ編集・カード情報
    this.contents.clear();
    if (this._card.id() > 0) {
      ...
      var unitSkillB = $dataSkills[this._card.unitSkillB()];
      if (unitSkillB) this.drawSkill(unitSkillB, x2, lineHeight * 12, width2);
      var unitSkillC = $dataSkills[this._card.unitSkillC()];
      if (unitSkillC) this.drawSkill(unitSkillC, x2, lineHeight * 15, width2);
      var partySkill = $dataSkills[this._card.partySkill()];
  ...
  Scene_CardBattle.prototype.createMessageWindow = function() {
    this._messageWindow = new Window_CardBattleMessage(0, 544);
